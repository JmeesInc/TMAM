...
class VideoSegModel2(nn.Module):
    def __init__(self, encoder, decoder, seg_head, device="cuda", sam2_predictor=None, batch_size=1, index = [i for i in range(60)], depth=5,  image_size = (1024, 1024)):
        super(VideoSegModel2, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.depth = depth
        self.seg_head = seg_head
...
class VideoSegModel_swin(nn.Module):
    def __init__(self, encoder, decoder, seg_head, device="cuda", sam2_predictor=None, batch_size=1, index = [i for i in range(60)], depth=5,  image_size = (1024, 1024)):
        super(VideoSegModel_swin, self).__init__()
        ...
        with torch.inference_mode():
            # テンソル形状は元コードに合わせて適宜
            features = self.encoder(torch.zeros(batch_size, 3, image_size[0], image_size[1], device=device))[self.depth-1]
    ...
    def forward(self, img):
        ...
        # 例: pix_feat = [0] + self.encoder(img)[:4]
        pix_feat = self.encoder(img)[:self.depth]
        pix_feat = [0] + [x.permute(0, 3, 1, 2) for x in pix_feat]
